#!/usr/bin/env python3
"""
MedReg AI - Streamlit Prototype
A simple web interface showcasing the core features of MedReg AI.
"""
...
# --- CLOUD RUN PATH FIX ---
# Add the current directory to Python path so imports work in Docker
...
def main():
    # Header
    st.markdown('<h1 class="main-header">🏥 MedReg AI: Automated Compliance</h1>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        **3. Export & Traceability** 📊
        ...
        # Check if API Key exists in Environment (Cloud Run Secrets)
        env_key = os.getenv("GOOGLE_AI_API_KEY")
        
        if env_key:
            st.session_state.api_key = env_key
            st.success("✅ API Key loaded from Cloud Environment")
        else:
